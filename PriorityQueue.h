...
private:
    ...
    void upheap(size_type index) {
        if (index < 1) { //When the index is back to 1 end recursion
            return;
        }
        int par = parent(index);
        if (comp(c.at(index), c.at(par))) { // When parent is greater than child, stop
            return;
        }
        std::swap(c.at(index),c.at(par));
        upheap(par); //Recur with the next parent
    }
    ...
    void downheap(size_type index) {
        if (is_leaf(index)) { //When index reaches outside
            return;
        }
        ...
        if (comp(c[index], c[ex])) {
            std::swap(c[ex],c [index]);
            downheap(ex);
        }
        return;
    }
...
public:
    ...
    void push( const value_type& value ) {
        c.push_back(value); //Add element to the back of the heap
        upheap(c.size()-1); // Call the upheap function on the newest element
    }
	...
	void push( value_type&& value ) {
        c.push_back(std::move(value)); //Add element to the back of the heap
        upheap(c.size()-1);
    }
